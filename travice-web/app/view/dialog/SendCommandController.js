...
Ext.define('Travice.view.dialog.SendCommandController', {
    extend: 'Travice.view.dialog.SavedCommandController',
    ...
    onSendResult: function (options, success, response) {
        if (success) {
            this.closeView();
            Travice.app.showToast(response.status === 202 ? Strings.commandQueued : Strings.commandSent);
        } else {
