...
copy_wrapper () {
  ...
  echo "wrapper.app.parameter.1=./conf/travice.xml" >> out/conf/wrapper.conf
  ...
  echo "wrapper.ntservice.name=travice" >> out/conf/wrapper.conf
  echo "wrapper.ntservice.displayname=Travice" >> out/conf/wrapper.conf
  echo "wrapper.ntservice.description=Travice" >> out/conf/wrapper.conf
...
copy_files () {
  ...
  cp -r ../travice-web/web/* out/web
  cp default.xml out/conf
  cp travice.xml out/conf
}
...
package_windows () {
  ...
  wine app/ISCC.exe travice.iss

  zip -j travice-windows-$VERSION.zip Output/travice-setup.exe README.txt
...
package_unix () {
  ...
  cp setup.sh out
  makeself --notemp out travice.run "travice" ./setup.sh

  zip -j travice-linux-$VERSION.zip travice.run README.txt
  cp travice-linux-$VERSION.zip travice-macos-$VERSION.zip
...
package_universal () {
  ...
  cp other/travice.sh out
  
  cd out
  zip -r ../travice-other-$VERSION.zip *
  cd ..
